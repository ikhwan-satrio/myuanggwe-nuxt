...
const isOwner = computed(() => {
  return (
    orgs.value.data?.members.find((o) => o.user.id == user.value.data?.user.id)
      ?.role === "owner"
  );
});

// Mutation untuk menghapus anggota menggunakan authClient
...
// Mutation untuk mengupdate role anggota menggunakan authClient
...
const deleteOrgMutation = useMutation({
  mutationFn: async () => {
    ...
    if (error) {
      throw new Error(error.message || "Cannot delete orgs!");
    }
    return data;
  },
  onSuccess: async () => {
...
const handleRemoveMember = (id: string) => {
  if (memberToRemove.value) {
    removeMemberMutation.mutate(memberToRemove.value.id);
  }
};
...
useHead({
  title: "Organization Management - MyUangGwe",
});
...
<template>
  <div class="container mx-auto max-w-4xl space-y-6 p-6">
    ...
    <div
      v-if="manageQuery.pending.value"
    ...
    <UiCard
      v-else-if="!manageQuery.data?.value"
      class="border-dashed bg-muted/50"
    >
      <UiCardContent
      ...
      >
        ...
        <div class="space-y-2">
          ...
          <p class="max-w-sm text-muted-foreground">
            Please switch to an organization context to manage its members and
            settings.
          </p>
        </div>
        <UiButton variant="outline" @click="navigateTo('/organizations')"
          >Go to Organizations</UiButton
        >
      </UiCardContent>
    </UiCard>
      ...
      >
        ...
        <div class="space-y-2">
          ...
          <p class="max-w-sm text-destructive/80">
            You must be an <strong>Owner</strong> of this organization to manage
    ...
    <template v-else>
      <UiCard>
        <UiCardHeader>
          ...
          <UiCardDescription
            >Manage who has access to this organization.</UiCardDescription
          >
        </UiCardHeader>
        <UiCardContent>
          <UiTable>
            ...
            <UiTableBody>
              <UiTableRow
                v-for="member in manageQuery.data?.value?.members"
                :key="member.id"
              >
      ...
      <UiCard class="border-destructive/50 bg-destructive/5">
        <UiCardHeader>
          ...
          <UiCardDescription
            >Irreversible actions for your organization.</UiCardDescription
          >
        </UiCardHeader>
        <UiCardContent>
          <div class="flex items-center justify-between">
            ...
            >
              <Icon
                v-if="deleteOrgMutation.isPending.value"
                name="lucide:loader-2"
                class="mr-2 h-4 w-4 animate-spin"
              />
              {{
                deleteOrgMutation.isPending.value
                  ? "Deleting..."
                  : "Delete Organization"
              }}
            </UiButton>
      ...
      <UiDialog
        ...
        @update:open="(open) => (isDeleteDialogOpen = open)"
      >
        <UiDialogContent>
          <UiDialogHeader>
          ...
          <UiDialogFooter>
            <UiButton variant="outline" @click="memberToRemove = null"
              >Cancel</UiButton
            >
            <UiButton
              variant="destructive"
            ...
            >
              <Icon
                v-if="removeMemberMutation.isPending.value"
                name="lucide:loader-2"
                class="mr-2 h-4 w-4 animate-spin"
              />
              Remove Member
        ...
        <UiDialogContent>
          <UiDialogHeader>
            <UiDialogTitle class="text-destructive"
              >Delete Organization</UiDialogTitle
            >
            <UiDialogDescription>
              <div class="space-y-3">
                <p>
                  CRITICAL ACTION: Are you absolutely sure you want to delete
                  this organization?
                </p>
                <div
                  class="rounded-md bg-destructive/10 p-3 text-sm font-medium text-destructive"
                >
                  Warning: All wallets, transactions, and categories associated
                  with this organization will be PERMANENTLY lost.
                </div>
              </div>
            </UiDialogDescription>
          </UiDialogHeader>
